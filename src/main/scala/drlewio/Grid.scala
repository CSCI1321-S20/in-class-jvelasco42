...
class Grid {
  ...
  private var _currentPill = new Pill(
    Seq(
      new PillPiece(3, 0, ColorOption.randomColor),
      new PillPiece(4, 0, ColorOption.randomColor)
    )
  )
  private var _nextPill = new Pill(
    Seq(
      new PillPiece(3, 0, ColorOption.randomColor),
      new PillPiece(4, 0, ColorOption.randomColor)
    )
  )
  private var _elememts: Seq[Element] =
    (for (i <- 0 until 8; j <- 6 until 16; if math.random < odds) yield {
  ...
  def update(delay: Double): Unit = {
    ...
    if (moveDelay >= moveInterval) {
      ...
      if (keysHeld(KeyCode.Left))
        _currentPill = currentPill.move(-1, 0, isClear)
      if (keysHeld(KeyCode.Right))
        _currentPill = currentPill.move(1, 0, isClear)
      moveDelay = 0.0
    }
  }
  ...
  def isClear(x: Int, y: Int): Boolean = {
    y < 16 && x >= 0 && x < 8 &&
    !_elememts.exists(e => e.cells.exists(c => c.x == x && c.y == y))
  }
